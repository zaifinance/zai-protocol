  ...
  describe('purchaseCoupons', function () {
    describe('before call', function () {
      ...
      it('shows correct potential coupon premium', async function () {
        expect(await this.market.couponPremium(100000)).to.be.bignumber.equal(
          new BN(3703),
        )
      })
    })
  ...
  describe('redeemCoupons', function () {
    ...
    describe('after redeemable', function () {
      ...
      describe('not enough coupon balance', function () {
        it('reverts', async function () {
          await expectRevert(
            this.market.redeemCoupons(1, 200000, { from: userAddress }),
    ...
    describe.skip('after expired', function () {
      this.timeout(30000)

      beforeEach(async function () {
        for (let i = 0; i < COUPON_EXPIRATION; i++) {
          await this.market.incrementEpochE()
        }
        await this.market.stepE()
      })
  ...
  describe.skip('step', function () {
    ...
    describe('on call without expiration', function () {
      it('initializes coupon expiry', async function () {
        expect(await this.market.couponsExpiration(2)).to.be.bignumber.equal(
          new BN(92),
        )
        expect(await this.market.expiringCoupons(92)).to.be.bignumber.equal(
          new BN(1),
        )
        expect(

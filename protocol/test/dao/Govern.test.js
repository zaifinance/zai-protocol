...
const VOTE_PERIOD = 144
const EXPIRATION = 48
const EMERGENCY_COMMIT_PERIOD = 96
...
describe('Govern', function () {
  ...
  describe('vote', function () {
    describe('cant vote', function () {
      ...
      describe('when ended', function () {
        ...
        it('is frozen', async function () {
          expect(await this.govern.statusOf(userAddress)).to.be.bignumber.equal(
            new BN(0),
          )
        })
    ...
    describe('can vote', function () {
      ...
      describe('when vote', function () {
        beforeEach(async function () {
          this.result = await this.govern.vote(this.implB.address, APPROVE, {
            from: userAddress,
          })
        ...
        it('is nominated', async function () {
          expect(await this.govern.isNominated(this.implB.address)).to.be.equal(
            true,
          )
        })
        ...
        it('records vote', async function () {
          expect(
            await this.govern.recordedVote(userAddress, this.implB.address),
      ...
      describe('when multiple vote', function () {
        ...
        it('sets vote counter correctly', async function () {
          expect(
            await this.govern.approveFor(this.implB.address),
          ).to.be.bignumber.equal(new BN(2000).mul(INITIAL_STAKE_MULTIPLE))
          expect(
            await this.govern.rejectFor(this.implB.address),
          ).to.be.bignumber.equal(new BN(1000).mul(INITIAL_STAKE_MULTIPLE))
        })
        ...
        it('records vote', async function () {
          ...
          expect(
            await this.govern.recordedVote(userAddress2, this.implB.address),
          ).to.be.bignumber.equal(REJECT)
          expect(
            await this.govern.recordedVote(userAddress3, this.implB.address),
          ).to.be.bignumber.equal(APPROVE)
        })
      ...
      describe('when revote', function () {
        beforeEach(async function () {
          ...
          this.result = await this.govern.vote(this.implB.address, REJECT, {
            from: userAddress,
          })
          this.txHash = this.result.tx
        })
  ...
  describe('commit', function () {
    ...
    describe('before nomination', function () {
      ...
      it('reverts', async function () {
        await expectRevert(
          this.govern.commit(this.implB.address, { from: userAddress }),
    ...
    describe('ends successfully', function () {
      ...
      it('is updated', async function () {
        ...
        expect(await this.govern.isInitialized(this.implB.address)).to.be.equal(
          true,
        )
      })
    ...
    describe('expired', function () {
      beforeEach(async function () {
        ...
        for (let i = 0; i < VOTE_PERIOD + EXPIRATION + 1; i++) {
          await this.govern.snapshotTotalBondedE()
          await this.govern.incrementEpochE()
        }
      })
  ...
  describe('emergency commit', function () {
    ...
    describe('ends successfully', function () {
      beforeEach(async function () {
        ...
        this.result = await this.govern.emergencyCommit(this.implB.address, {
          from: userAddress,
        })
        this.txHash = this.result.tx
      })

...
const POOL_REWARD_PERCENT = 50
const TREASURY_REWARD_BIPS = 0
...
function lessPoolAndTreasuryIncentive(baseAmount, newAmount) {
  return new BN(
    baseAmount +
      newAmount -
      poolIncentive(newAmount) -
      treasuryIncentive(newAmount),
  )
}
  ...
  describe('after bootstrapped', function () {
        ...
        describe('on step', function () {
          ...
          it('mints new Dollar tokens', async function () {
            ...
            expect(
              ...
              lessPoolAndTreasuryIncentive(1000000, this.expectedReward),
            )
            expect(
              await this.dollar.balanceOf(poolAddress),
            ).to.be.bignumber.equal(poolIncentive(this.expectedReward))
            expect(
              await this.dollar.balanceOf(TREASURY_ADDRESS),
            ).to.be.bignumber.equal(treasuryIncentive(this.expectedReward))
          })

          it('updates totals', async function () {
            expect(await this.regulator.totalStaged()).to.be.bignumber.equal(
              new BN(0),
            )
            expect(await this.regulator.totalBonded()).to.be.bignumber.equal(
              lessPoolAndTreasuryIncentive(1000000, this.expectedReward),
            )
            expect(await this.regulator.totalDebt()).to.be.bignumber.equal(
              new BN(0),
            )
            expect(await this.regulator.totalSupply()).to.be.bignumber.equal(
              new BN(0),
            )
            expect(await this.regulator.totalCoupons()).to.be.bignumber.equal(
              new BN(0),
            )
            expect(
      ...
      describe('(1) - refresh redeemable at specified ratio', function () {
        ...
        describe('on step', function () {
          beforeEach(async function () {
            await this.oracle.set(101, 100, true)
            this.expectedReward = 10000
            this.expectedRewardCoupons = 5000
            this.expectedRewardDAO = 0
            this.expectedRewardLP = 5000
            this.expectedRewardTreasury = 0
          ...
          it('mints new Dollar tokens', async function () {
            ...
            expect(
              ...
              new BN(1000000).add(new BN(this.expectedRewardCoupons)),
            )
            expect(
              await this.dollar.balanceOf(poolAddress),
            ...
            expect(
              await this.dollar.balanceOf(TREASURY_ADDRESS),
            ).to.be.bignumber.equal(new BN(this.expectedRewardTreasury))
          })
    ...
    describe('(1 + 2) - refresh redeemable then mint to bonded', function () {
      ...
      describe('on step', function () {
        beforeEach(async function () {
          await this.oracle.set(101, 100, true)
          this.bondedReward = 3000
          this.newRedeemable = 2000
          this.poolReward = 5000
          this.treasuryReward = 0
        ...
        it('mints new Dollar tokens', async function () {
          ...
          expect(
            ...
            new BN(1000000 + this.newRedeemable + this.bondedReward),
          )
          expect(
            await this.dollar.balanceOf(poolAddress),
          ).to.be.bignumber.equal(new BN(this.poolReward))
          expect(
            await this.dollar.balanceOf(TREASURY_ADDRESS),
          ).to.be.bignumber.equal(new BN(this.treasuryReward))
        })
        ...
        it('updates totals', async function () {
          ...
          expect(await this.regulator.totalBonded()).to.be.bignumber.equal(
            new BN(1000000 + this.bondedReward),
          )
          expect(await this.regulator.totalDebt()).to.be.bignumber.equal(
            new BN(0),
          )
          ...
          expect(await this.regulator.totalRedeemable()).to.be.bignumber.equal(
            new BN(2000),
          )
        })
    ...
    describe('(3) - above limit but below coupon limit', function () {
      ...
      describe('on step', function () {
        beforeEach(async function () {
          ...
          this.expectedRewardCoupons = 10000
          this.expectedRewardDAO = 0
        ...
        describe('on step', function () {
          ...
          it('updates totals', async function () {
            it('updates totals', async function () {
              ...
              expect(await this.regulator.totalDebt()).to.be.bignumber.equal(
                new BN(345000).add(new BN(this.expectedDebt)),
              )
              expect(await this.regulator.totalSupply()).to.be.bignumber.equal(
                new BN(0),
              )
        ...
        describe('on step', function () {
          ...
          it('updates totals', async function () {
            it('updates totals', async function () {
              ...
              expect(await this.regulator.totalDebt()).to.be.bignumber.equal(
                new BN(350000).add(new BN(this.expectedDebt)),
              )
              expect(await this.regulator.totalSupply()).to.be.bignumber.equal(
                new BN(0),
              )
